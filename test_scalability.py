...
class SyntheticAreaDetector:
    ...
    def __init__(
        ...
        center_2theta: float,
        center_gamma: float,
        width_2theta: float = 10.0,
        width_gamma: float = 15.0,
        ...
        self.limits = (
            np.deg2rad(twoth_min),
            np.deg2rad(twoth_max),
            np.deg2rad(gamma_min),
            np.deg2rad(gamma_max),
        )
        ...
        class MockImage:
            def __init__(self):
                self.header = {
                    "TITLE": f"Synthetic detector at 2θ={center_2theta:.1f}°, γ={center_gamma:.1f}°"
                }
    ...
    for n_detectors in test_cases:
        ...
        try:
            ...
            for i, det in enumerate(detectors):
                ...
                print(
                    f"  Detector {i}: 2θ=[{twoth_range[0]:.1f}, {twoth_range[1]:.1f}]°, "
            ...
            unique_weights, counts = np.unique(
                weight_grid[weight_grid > 0], return_counts=True
            )
            print("\nWeight distribution:")
            ...
            try:
                _, hull_weight_grid, hull_normalized_data = merger.merge_convex_hull(
                    verbose=True
                )

                # Check if convex hull worked properly
                hull_unique_weights, hull_counts = np.unique(
                    hull_weight_grid[hull_weight_grid > 0], return_counts=True
                )
...
def analyze_weight_logic():
    ...
    print(
        f"  Single detector: {single_coverage:,} pixels ({100 * single_coverage / total_pixels:.1f}%)"
    )
    print(
        f"  Two detectors overlap: {dual_overlap:,} pixels ({100 * dual_overlap / total_pixels:.1f}%)"
    )
    print(
        f"  Three detectors overlap: {triple_overlap:,} pixels ({100 * triple_overlap / total_pixels:.1f}%)"
    )
    ...
    print(
        f"  But with 3 detectors, we also have weight=3 regions: {triple_overlap:,} pixels"
    )
    print(f"  Total overlap regions: {dual_overlap + triple_overlap:,} pixels")

    if triple_overlap > 0:
        print(
            f"  ⚠️  Convex hull method will miss {100 * triple_overlap / (dual_overlap + triple_overlap):.1f}% of overlap regions!"
        )
