...
class AreaDetectorImageConverter(AreaDetectorImage):
    ...
    def set_converted_data_with_coordinates(
        self, data_converted, gamma_coords, twoth_coords
    ):
        """
    ...
    def convert_back_to_detector(self, n_row=None, n_col=None, method="nearest"):
        ...
        if len(self.indexes[0]) == 0 or len(self.indexes[1]) == 0:
            raise ValueError(
                "Coordinate indexes not properly set. Use set_converted_data_with_coordinates() first."
            )
        ...
        new_twoth, new_gamma = self.rowcol_to_angles(
            *np.meshgrid(seq_row, seq_col, indexing="ij"),
        ...
        if hasattr(self, "verbose") and self.verbose:
            ...
            print(
                f"  New 2theta: [{new_twoth_deg.min():.2f}, {new_twoth_deg.max():.2f}]"
            )
            print(
                f"  New gamma: [{new_gamma_deg.min():.2f}, {new_gamma_deg.max():.2f}]"
            )
        ...
        new = (
            f(np.c_[new_gamma.ravel(), new_twoth.ravel()])
            .reshape((n_row, n_col))
            .astype(self.data_converted.dtype)
        )
