...
class ImprovedAreaDetectorMerger:
    ...
    def merge_convex_hull_improved(
        self, verbose: bool = True
    ) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """
        ...
        if verbose:
            print(
                f"=== Improved Convex Hull Merging ({self.n_detectors} detectors) ==="
            )
        ...
        if verbose:
            ...
            print(
                f"All overlap regions (weight >= {self.overlap_threshold}): {total_overlap_points:,} points"
            )
        ...
        if verbose:
            print(
                f"✓ Improved convex hull merge {'completed' if hull_success else 'failed'}"
            )
            if hull_success:
    ...
    def _apply_improved_hull_weights(
        self, inside_hull_2d: np.ndarray, verbose: bool = True
    ) -> None:
        """
        ...
        if verbose:
            ...
            for w in range(1, self.n_detectors + 1):
                ...
                if orig_count > 0 or new_count > 0:
                    ...
                    if change != 0:
                        print(
                            f"    Weight {w}: {orig_count:,} → {new_count:,} ({change:+,})"
                        )
                    else:
    ...
    def _print_coverage_analysis(self) -> None:
        ...
        if overlap_pixels > 0:
            ...
            print(
                f"  Total overlap: {overlap_pixels:,} pixels ({overlap_percentage:.1f}%)"
            )
    ...
    def get_detailed_quality_metrics(self) -> Dict[str, Any]:
        ...
        for w in range(1, self.n_detectors + 1):
            ...
            if original_count > 0:
                metrics[f"weight_{w}_change_pct"] = (
                    100 * (hull_count - original_count) / original_count
                )
        ...
        if original_total_overlap > 0:
            metrics["total_overlap_expansion_factor"] = (
                hull_total_overlap / original_total_overlap
            )
        else:
