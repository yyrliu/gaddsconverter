...
def find_peaks_and_widths(twoth_values, intensity, smooth=True, width_at_height=0.8, wlen=None, find_peaks_dict={"prominence": 100}):
    """
    ...
    for i in range(len(peaks)):
        peak_intensity = np.max(intensity[widths_result[2][i].astype(int): widths_result[3][i].astype(int) + 1])
        properties = {
...
def correct_baseline(twoth_values, intensity, offset=100, min_clip=None, method=None, **kwargs):
    """
    ...
    **kwargs : dict
    ...
    baseline_fitter = Baseline(x_data=twoth_values, check_finite=False, assume_sorted=True)
    func = getattr(baseline_fitter, method)
    calc_baseline, params = func(intensity, **kwargs)
    corrected_intensity = np.clip(intensity - calc_baseline, a_min=min_clip, a_max=None) + offset
