...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.ui.dsbGridWidth.valueChanged.connect(
            lambda lw: self.set_grid("linewidth", lw)
        )
        self.ui.cbGridStyle.currentTextChanged.connect(
            lambda ls: self.set_grid("linestyle", ls)
        )
        ...
        self.convertFinished.connect(
            lambda: self.ui.tabPlots.setCurrentWidget(self.ui.plotConverted)
        )
    ...
    def open_file(self, *, f=None):
        if f is None:
            f, _ = QtWidgets.QFileDialog.getOpenFileName(
                self, "select file", "", "Image files (*.gfrm *.sfrm);; All files (*)"
            )
...
See GADDS User Manual for details.""",
    ...
    def plot_converted(self, log_norm=True):
        ...
        extent = (
            self.gfrm.indexes[1][0] - dx / 2,
            self.gfrm.indexes[1][-1] + dx / 2,
            self.gfrm.indexes[0][-1] - dy / 2,
            self.gfrm.indexes[0][0] + dy / 2,
        )
        ...
        if self.ui.cbGrid2th.isChecked():
            ...
            for angle_deg in np.arange(delta_deg, 180, delta_deg):
                if self.gfrm.limits[0] <= np.deg2rad(angle_deg) <= self.gfrm.limits[1]:
                    ax.plot(
                        *self.gfrm.gridline(angle_deg, "twoth"), ls, lw=lw, color=color
                    )
        if self.ui.cbGridGamma.isChecked():
            delta_deg = self.ui.sbGridGamma.value()
            for angle_deg in np.concatenate(
                (
                    np.arange(-90, 180, delta_deg),
                    np.arange(-90 - delta_deg, -180, -delta_deg),
                )
            ):
                if self.gfrm.limits[2] <= np.deg2rad(angle_deg) <= self.gfrm.limits[3]:
                    ax.plot(
                        *self.gfrm.gridline(angle_deg, "gamma"), ls, lw=lw, color=color
                    )
        self.ui.plotOriginal.canvas.draw()
    ...
    def save_original(self, *, f=None):
        ...
        if f:
            if self.gfrm.scale != 1 or self.gfrm.offset != 0:
                data = (
                    self.gfrm.image.data.astype(np.float32) * self.gfrm.scale
                    + self.gfrm.offset
                )
            else:
                data = self.gfrm.image.data
    ...
    def save_grids(self, *, f=None):
        ...
        if not ax.lines:
            QtWidgets.QMessageBox.warning(
                self, "error", "no grid (twoth, gamma) is checked."
            )
            return
        ax.set_aspect(1)
        ax.set_xticks([])
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--batch", help="command line mode (batch operation)", action="store_true"
    )
