...
class AreaDetectorMerger:
    ...
    def __init__(self, area_detectors: List[AreaDetectorImage]):
        ...
        if len(area_detectors) > 2:
            raise NotImplementedError(
                "Only tested with 2 detectors so far, for more than 2 detectors, please only use the data-driven method."
            )

        detector_chis = [detector.chi for detector in area_detectors]
        if not detector_chis.count(detector_chis[0]) == len(detector_chis):
            raise NotImplementedError(
                "All detectors must have the same chi angle for merging."
            )
    ...
    def _setup_common_grid(self) -> None:
        ...
        twoth_step = min(
            det.indexes[1][1] - det.indexes[1][0] for det in self.area_detectors
        )
        gamma_step = min(
            det.indexes[0][1] - det.indexes[0][0] for det in self.area_detectors
        )
    ...
    def merge_data_driven(
        ...
        twoth_mesh, gamma_mesh = np.meshgrid(
            self.common_grid["twoth"], self.common_grid["gamma"], indexing="xy"
        )
        ...
        for i, area_detector in enumerate(self.area_detectors):
            ...
            final_coverage_mask = data_coverage_mask | (
                coverage_mask & (interpolated_data >= 0)
            )

            self.detector_coverage_masks.append(final_coverage_mask)

            # Accumulate data and weights
            self.merged_data[final_coverage_mask] += interpolated_data[
                final_coverage_mask
            ]
    ...
    def merge_convex_hull(
        ...
        if verbose:
            ...
            print(
                f"  2θ range: [{np.min(overlap_twoth_coords):.2f}, {np.max(overlap_twoth_coords):.2f}]°"
            )
            print(
                f"  γ range: [{np.min(overlap_gamma_coords):.2f}, {np.max(overlap_gamma_coords):.2f}]°"
            )
        ...
        if len(overlap_gamma_coords) > 3:  # Need at least 3 points for hull
            try:
                ...
                overlap_points_2d = np.column_stack(
                    (overlap_twoth_coords, overlap_gamma_coords)
                )
                ...
                expansion_pct = (
                    100 * additional_points / original_overlap_points
                    if original_overlap_points > 0
                    else 0
                )
                ...
                if verbose:
                    ...
                    print(
                        f"  Additional points: {additional_points:,} ({expansion_pct:.1f}% increase)"
                    )
        ...
        return (
            self.merged_data,
            self.convex_hull_weight_grid,
            self.convex_hull_normalized_data,
        )
    ...
    def get_detector_info(self) -> List[Dict[str, Any]]:
        ...
        for i, area_detector in enumerate(self.area_detectors):
            info = {
                ...
                "twoth_range": (
                    area_detector.indexes[1].min(),
                    area_detector.indexes[1].max(),
                ),
                "gamma_range": (
                    area_detector.indexes[0].min(),
                    area_detector.indexes[0].max(),
                ),
            }
    ...
    def print_detector_info(self) -> None:
        ...
        for info in detector_info:
            ...
            print(
                f"2θ range: [{info['twoth_range'][0]:.2f}, {info['twoth_range'][1]:.2f}]°"
            )
            print(
                f"γ range: [{info['gamma_range'][0]:.2f}, {info['gamma_range'][1]:.2f}]°"
            )
            print("=== end ===\n")
    ...
    def get_quality_metrics(self) -> Dict[str, float]:
        ...
        if self.normalized_data is None or self.convex_hull_normalized_data is None:
            raise ValueError(
                "Both merging methods must be run before calculating metrics"
            )
        ...
        expansion_factor = (
            hull_overlap / original_overlap if original_overlap > 0 else 0
        )
        ...
        metrics = {
            ...
            "hull_vertices": len(self.hull_points)
    ...
    def create_comparison_plot(
        self, figsize: Tuple[int, int] = (24, 16), save_path: Optional[str] = None
    ) -> plt.Figure:
        """
        ...
        if self.normalized_data is None or self.convex_hull_normalized_data is None:
            raise ValueError(
                "Both merging methods must be run before creating comparison plot"
            )
    ...
    def _plot_data_comparison(self, fig: plt.Figure, extent: List[float]) -> None:
        ...
        data_difference[self.weight_grid == 0] = np.nan
        im7 = ax7.imshow(
            data_difference, cmap="RdBu_r", origin="upper", extent=extent, aspect="auto"
        )
    ...
    def _plot_intensity_data(
        self, ax: plt.Axes, data: np.ndarray, extent: List[float], title: str
    ) -> None:
        ...
        if np.nansum(data) > 0:
            ...
            if len(valid_data) > 0:
                ...
                im = ax.imshow(
                    data, cmap="viridis", origin="upper", extent=extent, aspect="auto"
                )
        else:
            im = ax.imshow(
                data, cmap="viridis", origin="upper", extent=extent, aspect="auto"
    ...
    def _plot_analysis(self, fig: plt.Figure) -> None:
        ...
        ax9.plot(
            self.common_grid["twoth"][valid_slice],
            slice_original[valid_slice],
            "b-",
            linewidth=2,
            label="Original",
            alpha=0.8,
        ...
        ax9.set_title(
            f"1D Comparison at γ = {self.common_grid['gamma'][gamma_center_idx]:.1f}°",
            fontsize=12,
        )
        ax9.legend()
    ...
    def _plot_statistics_comparison(self, ax: plt.Axes) -> None:
        ...
        bars1 = ax.bar(
            x - width / 2, stats_data["Original"], width, label="Original", alpha=0.8
        )
        bars2 = ax.bar(
            x + width / 2,
            stats_data["Convex Hull"],
            width,
            label="Convex Hull",
            alpha=0.8,
        )
...
"""
    ...
    def _add_grid_lines(self, fig: plt.Figure, extent: List[float]) -> None:
        ...
        for ax in image_axes:
            ...
            for gamma in range(
                int(gamma_min // 15) * 15, int(gamma_max // 15) * 15 + 15, 15
            ):
                if gamma_min <= gamma <= gamma_max:
                    ax.axhline(y=gamma, color="white", alpha=0.3, linewidth=0.5)


def load_and_merge_detectors(
    gfrm_files: List[str], method: str = "data_driven", verbose: bool = True
) -> AreaDetectorMerger:
    """
